...
import { PlaygroundPluginStartDeps, IndicesQuerySourceFields } from '../types';

export const useIndicesFields = (indices: string[]) => {
  const { services } = useKibana<PlaygroundPluginStartDeps>();
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const response = await services.http.post<IndicesQuerySourceFields>(
        '/internal/enterprise_search/playground/query_source_fields',
        {
          body: JSON.stringify({
            indices,
          }),
